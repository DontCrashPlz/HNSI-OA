...
public class DepartmentInfoTableHelper {
     ...
     * @return 成功的条目数
     */
    public synchronized long insertAll(List<RealDepartmentEntity> departments){
        ...
        for (RealDepartmentEntity department : departments) {
            ...
            values.put(MYSQLiteOpenHelper.TableDepartmentInfo.CHILDRENNUM, department.getNum());
            values.put(MYSQLiteOpenHelper.TableDepartmentInfo.TYPE, department.getType());
    ...
    public ArrayList<RealDepartmentEntity> queryAllDepaetment(){
        SQLiteDatabase readableDB= mySQLiteOpenHelper.getReadableDatabase();
        Cursor cursor= readableDB.rawQuery("select * from " + MYSQLiteOpenHelper.DB_TABLE_DEPARTMENT_INFO + " where _orgid > 0", null);

        ArrayList<RealDepartmentEntity> mDepartmentList= new ArrayList<>();
        if (cursor.moveToFirst()){
            while (!cursor.isAfterLast()){
                RealDepartmentEntity entity= createDepartmentEntity(cursor);
                if (entity != null){
     ...
     * @return
     */
    protected RealDepartmentEntity createDepartmentEntity(Cursor cursor){
        RealDepartmentEntity entity = null;
        if (cursor != null){
            entity = new RealDepartmentEntity();
            try {
                entity.setOrgid(cursor.getInt(cursor.getColumnIndex(MYSQLiteOpenHelper.TableDepartmentInfo.ORGID)));
                entity.setOrgname(cursor.getString(cursor.getColumnIndex(MYSQLiteOpenHelper.TableDepartmentInfo.ORGNAME)));
                entity.setType(cursor.getInt(cursor.getColumnIndex(MYSQLiteOpenHelper.TableDepartmentInfo.TYPE)));
                entity.setNum(cursor.getInt(cursor.getColumnIndex(MYSQLiteOpenHelper.TableDepartmentInfo.CHILDRENNUM)));
